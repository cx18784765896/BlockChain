...
func PrintUsage() {
	...
	fmt.Printf("\t createwallets --创建钱包集合 \n")
	fmt.Printf("\t walletsaddresses --获取钱包地址列表 \n")
...
func (cli *CLI) Run() {
	...
	getWalletsAddrCmd := flag.NewFlagSet("walletsaddresses",flag.ExitOnError)
	...
	case "walletsaddresses":
		err := getWalletsAddrCmd.Parse(os.Args[2:])
		if nil != err{
			log.Panicf("prase cmd getWalletsAddr failed! %v\n",err)
		}
	case "createwallets":
		err := createWalletsCmd.Parse(os.Args[2:])
		if nil != err{
			log.Panicf("prase cmd createwallets failed! %v\n",err)
		}
	case "printchain":
